...
async function pollJobResult(
  ...
  const timeoutMs = Math.max(
    30_000,
    Number.parseInt(process.env.TRYON_JOB_TIMEOUT_MS || "600000", 10)
  );
  const intervalMs = Math.max(
    500,
    Number.parseInt(process.env.TRYON_JOB_POLL_INTERVAL_MS || "1200", 10)
  );
  const startedAt = Date.now();

  while (Date.now() - startedAt < timeoutMs) {
    const statusUrl = new URL(
      `/api/jobs/${encodeURIComponent(queue)}/${encodeURIComponent(String(jobId))}`,
      baseUrl
    ).toString();
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `获取任务状态失败: ${response.status} ${response.statusText} ${text}`
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ✅ 安全修复：IP 速率限制 (5次/分钟 — 试穿更耗资源)
    const ip = request.headers.get("x-forwarded-for") || request.headers.get("x-real-ip") || "unknown";
    if (!checkRateLimit(`tryon:${ip}`, 5, 60_000)) {
      return NextResponse.json({ error: "试穿请求过于频繁，请稍后再试" }, { status: 429 });
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error: "创建试穿任务失败",
          details: `${createResp.status} ${createResp.statusText} ${text}`,
        },
